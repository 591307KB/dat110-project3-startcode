...
public class Util {
	...
	public static boolean computeLogic(BigInteger id, BigInteger lower, BigInteger upper) {
		...
		BigInteger lmod = lower.mod(bound);
		BigInteger umod = upper.mod(bound);
		BigInteger idmod = id.mod(bound);

		if (lower.compareTo(upper) > 0) {
			if (idmod.compareTo(umod) > 0 && idmod.compareTo(lmod) < 0)
				cond = false;
			else
				cond = true;
		} else if (idmod.compareTo(umod) <= 0 && idmod.compareTo(lmod) >= 0)
			cond = true;
		return cond;
	}
